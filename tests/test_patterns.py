...
def test_ipa_short_int():
    assert [6, 34, 26] == Dictionary.get_word_from_Dict("дом")[0].intipa
...
def test_shorten_pat_long_words_to_head_tail():
    ...
    head, tail = Patterns.shorten_pat_long_words_to_head_tail(pat_of_ipa, max_length_pat_of_ipa=8)
    assert ['cons', 'stress_v', 'cons', 'cons', 'vowel', 'cons', 'vowel', 'cons'] == head
    assert ['vowel', 'cons', 'vowel', 'cons'] == tail


def test_convert_tail_to_pattern():
    tail = ['vowel', 'cons', 'vowel', 'cons']
    assert ['same_v', 'same_cons', 'same_v', 'same_cons'] == Patterns.convert_tail_to_pattern(tail)


def test_add_tail_to_pats():
    all_rhyme_pats = [['add_sound', 'same_stressed_v']]
    tail_to_pattern = ['same_cons', 'same_v']
    assert [['add_sound', 'same_stressed_v', 'same_cons', 'same_v']] \
           == list(Patterns.add_tail_to_pats(all_rhyme_pats, tail_to_pattern))
...
def test_check_if_voice_palatal():
    ...
    ipa_short_int = Dictionary.get_word_from_Dict("дом")[0].intipa
    ...
    ipa_short_int = Dictionary.get_word_from_Dict("раб")[0].intipa
...
def test_check_n_mutations():
    ...
    assert bool(True) == Patterns.check_n_mutations(pat_true_1, max_number_hard_sounds_in_one_pat=2)
...
def test_condition_all_rhyme_pats():
    ipa_short_int = Dictionary.get_word_from_Dict("дом")[0].intipa
    all_rhyme_pats_1 = [["no_sound", "no_sound", "any_cons"], ['voice_cons', 'same_stressed_v', 'palatal_cons']]
    all_rhyme_pats_2 = [["no_sound", "no_sound", "any_cons"], ['voice_cons', 'same_stressed_v', 'voice_cons']]
    assert [['voice_cons', 'same_stressed_v', 'palatal_cons']] == \
           [item for item in Patterns.condition_all_rhyme_pats(all_rhyme_pats_1, ipa_short_int)]
    assert [["no_sound", "no_sound", "any_cons"]] == \
           [item for item in Patterns.condition_all_rhyme_pats(all_rhyme_pats_2, ipa_short_int,
...
def test_condition_add_sound_to_rhyme_pats():
    all_rhyme_pats = [["add_sound", "no_sound", "same_stressed_v"],
