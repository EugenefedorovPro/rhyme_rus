...
def test_find_rhymes_by_score():
    ...
    with open("tests/test_explore_rhymes/test_find_rhymes_by_score.pkl", "rb") as f:
...
def test_find_rhymes_by_pos():
    ...
    with open("tests/test_explore_rhymes/test_find_rhymes_by_pos.pkl", "rb") as f:
        assert dill.load(f).equals(short_table)
...
def test_rhymes_by_pattern():
    ...
    with open("tests/test_explore_rhymes/test_find_rhymes_by_pattern.pkl", "rb") as f:
        assert dill.load(f).equals(short_table)


def test_find_rhymes_by_word():
    short_table = ExploreRhymes.find_rhymes_by_word("тромб", romb_table)
    with open("tests/test_explore_rhymes/test_find_rhymes_by_word.pkl", "rb") as f:
