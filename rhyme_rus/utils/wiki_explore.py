...
class WikiExplore:
    ...
    def make_df_n_vowels_after_stress(cls):
        n_sounds_after_stress = [len(item.ipa) for item in Dictionary.instances
                                 if item.ipa and item.status]
        n_vowels_after_stress = [len(item.ipa.vowels) for item in Dictionary.instances
                                 if item.ipa and item.status]
        df_n_vowels_after_stress = (pd.DataFrame.from_dict({"vowels": n_vowels_after_stress,
    ...
    def get_items_by_n_vowels_after_stress(cls, n_vowels_after_stress,
                                           n_sounds_after_stress, n_of_words,
                                           random_choice=True):
        ...
        if random_choice:
            rand_n = [random.randint(0, len(items_by_n_vowels_after_stress)) for i in range(n_of_words)]
            items_by_n_vowels_after_stress = [item for i, item in enumerate
